...
class TAW_VDMenu {
	...
	class controls {
		...
		//Terrain/Grass Settings
		class VD_terrain_none_text : RscText {
			idc = TERRAIN_NONE_TEXT;
			style = 2;
			font = "PuristaLight";
			colorText[] = { 1, 1, 1, 1 };
			colorBackground[] = { 0, 0, 0, 0 };
		...
		class VD_terrain_low_text : VD_terrain_none_text {
			text = "Low";
			x = .675;
		};

		class VD_terrain_normal_text : VD_terrain_none_text {
			text = "Normal";
			x = .765;
		};

		class VD_terrain_high_text : VD_terrain_none_text {
			text = "High";
			x = .855;
		};

		class VD_terrain_none : RscButtonMenu {
			idc = TERRAIN_NONE;
			text = "";
			onButtonClick = "['none'] call TAWVD_fnc_onTerrainChanged;";
			colorBackground[] = {0,0,0,0};
		...
		class VD_terrain_low : VD_terrain_none {
			idc = TERRAIN_LOW;
			onButtonClick = "['low'] call TAWVD_fnc_onTerrainChanged;";
			x = .675;
		};

		class VD_terrain_normal : VD_terrain_none {
			idc = TERRAIN_NORMAL;
			onButtonClick = "['norm'] call TAWVD_fnc_onTerrainChanged;";
			x = .765;
		};

		class VD_terrain_high : VD_terrain_none {
			idc = TERRAIN_HIGH;
			onButtonClick = "['high'] call TAWVD_fnc_onTerrainChanged;";
			x = .855;
		};

		class TerrainUnderlineNone : RscText {
			idc = TERRAIN_UL_NONE;
			show = 0;
			text = "";
		...
		class TerrainUnderlineLow : TerrainUnderlineNone {
			idc = TERRAIN_UL_LOW;
			x = .696;
			w = .039;
		};

		class TerrainUnderlineNormal : TerrainUnderlineNone {
			idc = TERRAIN_UL_NORMAL;
			x = .771;
			w = .068;
		};

		class TerrainUnderlineHigh : TerrainUnderlineNone {
			idc = TERRAIN_UL_HIGH;
			x = .871;
			w = .049;
		};
		...
		class ObjectSyncCheckbox : RscCheckbox
		{
			...
			onCheckedChanged = "if((_this select 1) == 1) then {tawvd_syncObject = true;ctrlEnable [2941,false]; ctrlEnable [2942,false];} else {tawvd_syncObject = false; ctrlEnable [2942,true]; ctrlEnable [2941,true];}; [] call TAWVD_fnc_updateViewDistance; [] call TAWVD_fnc_persistSettingsSafe;";
			...
			class Controls {
				class SaveLoadGroup:RscControlsGroupNoScrollbars {
					...
					class Controls {
						...
						class SetDefaultButton:RscButtonMenu {
							text = "Set As Default";
							onButtonClick = "[] call TAWVD_fnc_onSetDefaultPressed;";
							x = .16;
							y = 0.57 - (1 / 25);
							w = (8 / 40);
							h = (1 / 25);
